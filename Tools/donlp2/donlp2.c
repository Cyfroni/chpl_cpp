...
#pragma omp threadprivate (donlp2_yy, o8bfgs_dg, o8bfgs_adx, o8bfgs_ltdx, o8bfgs_gtdx, o8bfgs_updx, o8bfgs_updz, \
	o8opti_qtx, o8opti_yy, o8opti_yx, o8opti_trvec, o8opti_con, o8opti_delist, o8opti_bindba, o8eval_con, \
	o8unim_step, o8dec_qri, o8dec_qri0, o8elim_qri, o8elim_y, o8elim_rhsscal, o8elim_col, o8sol_xl, o8upd_sdiag, \
	o8upd_rn1, o8upd_w, o8qpdu_g0, o8qpdu_ci0, o8qpdu_cii, o8qpdu_cei, o8qpdu_xd, o8qpdu_s, o8qpdu_z, o8qpdu_vr, \
	o8qpdu_xdold, o8qpdu_udold, o8qpdu_ai, o8qpdu_iai, o8qpdu_iaexcl, o8qpdu_aiold, o8qpdu_eqlist, o8qpdu_iqlist, \
	o8qpdu_y, o8qpdu_mult, o8qpdu_qpdel, escongrad_errloc, escongrad_fhelp1, escongrad_fhelp2, escongrad_fhelp3, \
	escongrad_fhelp4, escongrad_fhelp5, escongrad_fhelp6)
...
/* --------------->  step one of problem initialization by user               */
    ...
    intakt = FALSE;
    te0    = TRUE;   /* control running optimizer on stdout */
    te1    = FALSE;   /* information about iteration in compressed form */
    te2    = FALSE;   /* detailed output in case of trouble */
    te3    = FALSE;   /* print gradients and Hessian        */
    cold   = TRUE;
    big    = 1.e20 ; 
...
/******************************************************************************/
    ...
    for (i=1; i<=n; i++)
    {
       ...
       {
          corr = TRUE ;
          if ( llow[i] && lup[i]  ) x[i] = (up[i]+low[i])/2.0;
          if ( llow[i] && !lup[i] ) x[i] = low[i]+2.0*del0;
          if ( lup[i]  && !llow[i]) x[i] = up[i]-2.0*del0; 
       }
    }   
...
/*    and stored in gres[0][i]. done afterwards                          */
    ...
    for( j=1; j<= 2*(n+nlin+nonlin); j++ ) mlopt[j]=u[j];
    *fopt=fx;
    global_mem_free();
...
/*  but remember there are formally 2*nres constraints */
    ...
    for ( i = nlin+1 ; i<=nlin+nonlin ; i++ ) val[i] = FALSE ;    
...
void o8bfgs(void) {
    ...
    void    o8upd(DOUBLE **r,DOUBLE z[],DOUBLE y[],INTEGER n,LOGICAL *fail);
    DOUBLE  o8vecn(INTEGER nl,INTEGER nm,DOUBLE x[]);
...
void o8opti(void) 
{
    ...
    void    o8ht   (INTEGER id,INTEGER incr,INTEGER is1,
                    INTEGER is2,INTEGER m,
    ...
    void    o8rght (DOUBLE **a,DOUBLE b[],DOUBLE y[],
                    DOUBLE *yl,INTEGER n);
    void    o8left (DOUBLE **a,DOUBLE b[],DOUBLE y[],
                    DOUBLE *yl,INTEGER n);
    DOUBLE  o8vecn (INTEGER nl,INTEGER nm,DOUBLE x[]);
    DOUBLE  o8sc3 ( INTEGER i , INTEGER j , INTEGER k , 
                    DOUBLE **mat, DOUBLE x[] );
    DOUBLE  o8sc4 ( INTEGER i , INTEGER j , INTEGER k , 
            DOUBLE **mat) ;
    void    o8qpdu (void);
    void    esf    (DOUBLE x[],DOUBLE *fx);
    void    esgradf(DOUBLE x[],DOUBLE gradf[]);
    void    escon    (INTEGER type, INTEGER liste[], DOUBLE x[],
               DOUBLE constr[],
                LOGICAL	 errlist[]);
    void    escongrad( INTEGER liste[], INTEGER shift ,
            DOUBLE x[], DOUBLE **grad_constr);
    ...
    for ( i = 1 ; i <= n ; i++ )
    { res[2*i-1] = x[i]-ug[i];      /*  the scaled variables */
      res[2*i]   = og[i]-x[i] ;
    }
    for ( i = 1 ; i<=nlin ; i++ )
    {
      term = o8sc3(1,n,i,gres,x);  /*   the gradients of the linear         */
    ...
    escon(1,clist,x,o8opti_con,confuerr);
...
/* be active simultaneously                                                 */
    ...
    escongrad(clist,nlin,x,gres);
    for ( i = 1 ; i <= clist[0] ; i++ )
    {
       cgres[clist[i]+nlin] += 1 ;
       val[clist[i]+nlin] = TRUE ;
...
/* inactive  
    ...
    if ( phase == 2 && dnorm > sqrt(xnorm*epsmac)  && ! singul 
         && ! viobnd && nres > 0 ) 
    {
  ...
  /* if no error occured, computation of o8opti_yx is now to be completed */
    ...
    for (i = 1 ; i <= n ; i++) 
    {
        if ( fabs(difx[i]) >= epsx*(fabs(x[i])+tm2) ) k = 1;
    }
    if ( k == 0 ) 
    {
        cschgx = cschgx+1;
  ...
  /*  active at the new point but not at the old point                        */
    ...
    if ( dnorm == zero && qpterm == 1 && optite == three ) return;
    if ( dnorm <= epsx*(min(xnorm,one)+epsx) && qpterm < 0 ) 
    ...
    if ( qpterm >= 0 && dnorm <= tm2*epsx*(epsx+min(one,xnorm))) 
    {
        if ( upsi <= nres*delmin ) 
        {
            optite = six;
...
void o8eval(DOUBLE sigact,DOUBLE *sigres,LOGICAL *reject,LOGICAL *error) 
{
    ...
    void    esf(DOUBLE x[],DOUBLE *fx);
    void    escon(INTEGER i,INTEGER liste[] , DOUBLE x[],
                                 DOUBLE con[],INTEGER err[]);
    DOUBLE  o8sc3(INTEGER i, INTEGER j , INTEGER k , 
        DOUBLE **mat, DOUBLE x[]);
    void    user_eval(DOUBLE xvar[],INTEGER mode);
...
void o8unim(DOUBLE sig1th) {
    ...
    if ( sig*max(one,dnorm) >= sigsm ) goto L100;
    stptrm = -one;
    sig    = zero;
...
void o8elim(void) {
    DOUBLE  o8sc3b 
      (INTEGER n,INTEGER m,INTEGER j,DOUBLE **a,DOUBLE b[]);
    DOUBLE  o8vecn(INTEGER nl,INTEGER nm,DOUBLE x[]);
    void o8msg(INTEGER num);
     INTEGER  n1,n2;  
...
/* end computation of data of mi-th QP constraint data */
    ...
    if ( fabs(psid) <= tiny*(c1*c2+condr) || ip == 0) {
        ...
        for (i = 1 ; i <= nres ; i++) 
        {
            ...
            if ( w1[2*i-1] < w[2*i-1] || w1[2*i] < w[2*i] ) wlow = TRUE;
        }
        if ( wlow ) 
        {
            s1 = zero;
...
void escon( INTEGER call_type , INTEGER liste[], DOUBLE x[], DOUBLE constr[],
            LOGICAL errliste[])
{
void  econ(INTEGER call_type,INTEGER liste[],DOUBLE  xtr[],DOUBLE constr[],
            LOGICAL errliste[]);
INTEGER j    ;
...
/* ************************************************************************ */
    ...
    DOUBLE* d1_malloc(INTEGER size1, INTEGER init); 
     ...
     DOUBLE   *fusave,xhelp,xincr,sd1,sd2,sd3,d1,d2,d3;
     INTEGER  i,j;
     LOGICAL  changed;
     INTEGER  firstTime = 1;
